...
@Component({
  selector: 'app-feedperfil',
  templateUrl: './feedperfil.component.html',
  styleUrls: ['./feedperfil.component.css']
})
export class FeedperfilComponent implements OnInit {
  ...
  obtenerTweets() {
    ...
    this.tweetService.obtenerTweets().subscribe(
      (tweets: any[]) => {
        console.log('Tweets obtenidos sin filtro:', tweets); // Console.log para los tweets obtenidos antes del filtro

        // Filtrar los tweets para mostrar solo aquellos del perfil visitado
        this.tweets = tweets.filter(tweet => tweet.usuario.id === this.idEnUrl);

        console.log('Tweets filtrados:', this.tweets); // Console.log para los tweets filtrados
      },
      error => {
